...
This document describes a web application demonstrating the integration of Google's Agent to Agent (A2A) protocol for multi-agent orchestration where the agents are written in Python and Java. This sample makes use of a brand new Java SDK for A2A. The application features a host agent coordinating tasks between a Python remote agent and a Java remote agent that interact with various MCP servers to fulfill user requests.
...
- create .env file in `airbnb_agent` folder with the following content
```bash
GOOGLE_API_KEY="your_api_key_here" 
```

- create .env file in `weather_agent` folder with the following content
```bash
