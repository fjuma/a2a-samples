...
const contentEditorPrompt = ai.prompt('content_editor_agent');
  ...
  async execute(
    ...
    try {
      ...
      if (this.cancelledTasks.has(taskId)) {
        console.log(`[ContentEditorAgentExecutor] Request cancelled for task: ${taskId}`);
      ...
      console.info(`[ContentEditorAgentExecutor] Prompt response: ${responseText}`);
...
async function main() {
  ...
  const PORT = process.env.CONTENT_EDITOR_AGENT_JAVA_PORT || 10003;
  expressApp.listen(PORT, () => {
    console.log(`[ContentEditorAgentJS] Server using new framework started on http://localhost:${PORT}`);
    console.log(`[ContentEditorAgentJS] Agent Card: http://localhost:${PORT}/.well-known/agent.json`);
    console.log('[ContentEditorAgentJS] Press Ctrl+C to stop the server');
  });
}
